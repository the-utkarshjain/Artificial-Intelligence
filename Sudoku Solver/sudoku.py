...
def sudoku2fma(puzzle):
    ...
    C2 = AND([OR([S(c,r,n) for c in range(1,10)]) for r in range(1,10) for n in range(1,10)])
    
    # Every column has all numbers
    C3 = AND([OR([S(c,r,n) for r in range(1,10)]) for c in range(1,10) for n in range(1,10)])

 
    sqrt = int(10**0.5)
    C4 = AND([ OR([ S(c,r,n) for (r,c) in ([i*sqrt+ut, j*sqrt + cd] for i in range(0,sqrt) for j in range(0,sqrt))  ]) for ut in range(1,sqrt) for cd in range(1,sqrt) for n in range(1,10) ])
